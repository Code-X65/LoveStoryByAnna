...
const ProductCard = ({ product }) => {
  ...
  return (
    <Link to={`/details/${product.id}`} className="group cursor-pointer block">
      <div
      ...
      >
        <img
          src={product.images?.[0] || product.image1}
          alt={product.name}
          loading="lazy"
          className={`w-full h-full object-cover transition-all duration-700 ease-in-out ${isHovered ? 'opacity-0 scale-105' : 'opacity-100 scale-100'
            }`}
        />
        <img
          src={product.images?.[1] || product.images?.[0] || product.image2}
          alt={product.name}
          loading="lazy"
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button
            ...
            className={`p-2 bg-white hover:bg-pink-300 hover:text-white transition-colors ${isFavorite ? 'bg-pink-300 text-white' : ''
              }`}
          >
      ...
      <div className="text-center px-2">
        ...
        <p className="text-sm font-semibold text-gray-800">
          FROM ₦{(product.price || 0).toLocaleString()}
        </p>
      </div>
    </Link>
  );
        ...
        <div className="max-w-7xl mx-auto px-4 h-full">
          <div className="flex flex-col lg:flex-row gap-8 h-full">
                ...
                <div className="mb-6 pb-6 border-b border-gray-200">
                  ...
                  {expandedCategories.collection !== false && (
                    <div className="space-y-3">
                      {staticCollections.map((collection, idx) => (
                  ...
                  {expandedCategories.size !== false && (
                    <div className="grid grid-cols-2 gap-2">
                      {sizeRanges.map((size, idx) => (
                        <label
                          key={idx}
                          className={`flex items-center justify-center px-3 py-2 border cursor-pointer transition-all ${selectedFilters.size.includes(size.label)
                    ...
                    <div className="space-y-3">
                      {colorOptions.map((color, idx) => (
                        <label key={idx} className="flex items-center cursor-pointer group">
                          ...
                          />
                          <div
                            className={`w-8 h-8 border-2 mr-3 flex-shrink-0 transition-all ${selectedFilters.color.includes(color.label)
            ...
            <main className="flex-1 overflow-y-auto h-full py-8">
              <div className={`grid gap-6 ${viewMode === 'grid-large'
                ? 'grid-cols-2 md:grid-cols-3'
                : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4'
                }`}>
                {loading ? (
                  <div className="col-span-full py-20 flex flex-col items-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-pink-300 mb-4"></div>
